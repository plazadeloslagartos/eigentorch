...
class HDF5DataSetP300Avg(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if item == 0:
            return {'features': np.cov(np.float32(np.mean(features0, axis=0)[::ds])), 'label': np.float32(self.item0[1])}
        elif item == 1:
            return {'features': np.cov(np.float32(np.mean(features1, axis=0)[::ds])), 'label': np.float32(self.item1[1])}
...
class HDF5DataSetP300(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.downsample is not None:
            ...
            ds = 1
        sample = {'features': np.float32(np.cov(np.mean(data_epochs[:n_idx], axis=0)[::ds])), 'label': np.float32(item_label),
